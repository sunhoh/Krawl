...
/** 블로그 검색 */
export async function fetchNaverBlog(keyword: string): Promise<SerpSnapshot> {
  const json = await naverSearch('blog', keyword);
  return { total: json.total, items: toItems(json) };
}

/** 웹 검색 (병원/플랫폼) */
export async function fetchNaverWeb(keyword: string): Promise<SerpSnapshot> {
  const json = await naverSearch('webkr', keyword);
  return { total: json.total, items: toItems(json) };
}

/** 지식인 검색 (질문 패턴) */
export async function fetchNaverKin(keyword: string): Promise<SerpSnapshot> {
  const json = await naverSearch('kin', keyword);
  return { total: json.total, items: toItems(json) };
}

/** 지역 검색 (장소) */
export async function fetchNaverLocal(keyword: string): Promise<SerpSnapshot> {
  const json = await naverSearch('local', keyword);
  return { total: json.total, items: toItems(json) };
}
